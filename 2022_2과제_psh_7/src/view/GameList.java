...
public class GameList extends BaseFrame {
	...
	public GameList() {
		...
		var rs = rs(
				"select r_no, r_date, c_name, g_name, t_name from reservation r, theme t, genre g, cafe c where c.c_no = r.c_no and g.g_no=t.g_no and t.t_no = r.t_no and r_attend=0 and u_no =?",
				user.get(0));
		for (var r : rs) {
			m.addRow(r.toArray());
		}
