...
public class ProfilePage extends BasePage {
	...
	public ProfilePage(int no) {
		super("프로필");

		user = getRows("select * from user where u_no = ?", no).get(0);

		isMe = no == toInt(user.get(0));
		var exp = toInt(getOne("select count(*) from library where u_no = ?", no)) * 3
				+ getRows("select * from v2 where u_no = ? group by g_no having count(*) > 2", no).size() * 10;
		var level = lbl("경험치 : " + exp + " [등급 : " + g_gd[exp / 20] + "]", 2, 15);

		add(c = new JPanel(new BorderLayout(20, 20)));
		add(s = new JPanel(new GridLayout(0, 1, 5, 5)), "South");

		c.add(new JLabel(getIcon(user.get(8), 200, 200)), "North");
		...
		level.setIcon(getIcon("./datfiles/등급사진/" + (exp / 20) + ".jpg", 80, 80));
	...
	private void lib() {
		...
		for (var rs : getRows(
				"select g.g_no, g_img, g_name. g_genre, g_age, round(avg(r_score), 1), format(l_priec, '#,##0'), form game g, review r, library l where g.g_no = r.g_no and g.g_no l.g_no and l.u_no = ? group by g.g_no",
				user.get(0))) {
