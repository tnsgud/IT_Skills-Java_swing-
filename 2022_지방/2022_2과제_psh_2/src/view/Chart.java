	...
	public Chart() {
		...
		add(sz(c = new JPanel() {
			...
			protected void paintComponent(Graphics g) {
				...
				int sum = toInt(
						getResult("select count(*) from reservation r, cafe c where c.c_no=r.c_no and c.a_no=?", ano)
								.get(0).get(0));
				var rs = getResult(
		...
		for (var r : rs) {
			...
			w.add(img).setBounds(toInt(r.get(1)), toInt(r.get(2)), img.width, img.height);
			...
			marker.addMouseListener(new MouseAdapter() {
				...
				public void mousePressed(MouseEvent e) {
					markers.forEach(l -> l.isSelected = false);
					imgs.forEach(l -> l.isSelected = false);
					...
					imgs.stream().filter(l -> l.getName().equals(s.getName())).forEach(l -> {
						l.isSelected = true;
						ano = toInt(getResult("select a_no from area where a_name=?", l.getName()));
					});
