	...
	public Room() {
		...
		add(btn = btn("선택", a -> {
			if (a.getActionCommand().equals("선택")) {
				...
				var before = (Before) this.getWindowListeners()[1];
				var q = new Quiz();
				before.b = q;
				q.addWindowListener(before);
		...
		while (imgs.size() < 9) {
			...
			if (!imgs.containsKey(n)) {
				var l = new JLabel(img("퀴즈/" + n + ".jpg", 150, 150));
				l.setEnabled(false);
		...
		this.addWindowListener(new WindowAdapter() {
			public void windowClosing(java.awt.event.WindowEvent e) {
				if (btn.getText().equals("게임시작")) {
					var t = JOptionPane.showConfirmDialog(null, "게임 도중 나갈 시 게임은  다시 진행할 수 없습니다.\n나가시겠습니까?", "경고",
