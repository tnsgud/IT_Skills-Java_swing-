...
public class MainFrame extends JFrame {
	...
	void ui() {
		setSize(1000, 600);
		setIconImage(Toolkit.getDefaultToolkit().getImage("./지급자료/Covid.png"));
	...
	void removeNavigater() {
		try {
			...
			System.err.println("Nav not found!");
		}
		System.out.println("Nav success fully removed from frame");
...
class Navigater extends JPanel {
	...
	void ui() {
		...
		n.add(imglbl = new JLabel(BasePage.getIcon("./지급자료/유저사진/" + BasePage.uno + ".jpg", 130, 130)), "West");
		nc.add(BasePage.lbl(BasePage.uname, JLabel.LEFT, 13));
		nc.add(BasePage.lbl(BasePage.uage, JLabel.LEFT, 13));
		nc.add(BasePage.lbl(BasePage.ulocal, JLabel.LEFT, 13));
		...
		for (var navM : "로그아웃,마이페이지,검색,핫한 키워드,코로나 현황".split(",")) {
			...
			navlbl.addMouseListener(new MouseAdapter() {
				...
				public void mousePressed(MouseEvent e) {
					...
					} else if (me.getText().equals("핫한 키워드")) {
						BasePage.mf.swapPage(new FamousPage());
					} else if (me.getText().equals("코로나 현황")) {
