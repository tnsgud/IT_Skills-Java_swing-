...
public class MainPage extends BasePage {
	...
	public MainPage() {
		...
		add(c = new JPanel(new BorderLayout()) {
			...
			protected void paintComponent(Graphics g) {
				...
				for (int i = 0; i < 4; i++) {
					...
					g2.drawString(str[0], 135 + i * 130, 230 - h);
		...
		for (int i = 0; i < 4; i++) {
			...
			var tmp1 = new JPanel(new GridLayout(1, 0));
			...
			tmp1.add(lbl("<html><font color='orange'>" + String.format("%.1f", (double) cnt / tot * 100) + "%", 0, 20));
			tmp1.add(lbl("<html>누적 " + cnt + "<br>신규 " + getOne(
					"select count(*) from purdchase where date(date) = now() and shot = ?", LocalDate.now(), i + 1), 0,
					20));
	...
	private void data() {
		...
		for (int i = 0; i < 6; i++) {
			var rs = getRows(
					...
					date, date.getMonthValue(), date.getYear()).get(0).stream().map(a -> a.toString())
