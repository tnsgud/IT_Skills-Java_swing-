...
public class GamePage extends BasePage {
	...
	public GamePage() {
		...
		game = getRows("select *, dc_price from v1 where g_no = ?", gNo).get(0);

		add(new JScrollPane(main));

		main.add(c = sz(new JPanel(new BorderLayout()), 800, 300));
		main.add(s = sz(new JPanel(), 800, 50 * toInt(getOne("select count(*) from review where gNo = ?", gNo))),
				"South");
		...
		cc.add(lbl("평점 : " + getOne("select round(avg(r_score), 1) from review where gNo = ?", game.get(0)) + "점", 2));
		cc.add(lbl("가격 : " + (toInt(game.get(5)) == 0 ? "무료" : game.get(5) + "원"), 2));
		...
		if (getOne("select * from library where gNo = ? and u_no = ?", gNo, user.get(0)).isEmpty()) {
			if (toInt(game.get(5)) == 0) {
				cs.add(btn("구매하기", a -> {
					new InfoDialog(Stream.of(gNo).collect(Collectors.toList()));
				}));
			} else {
				for (var cap : "장바구니에 추가,구매하기".split(",")) {
					cs.add(btn(cap, a -> {
						if (cap.equals("장바구니에 추가")) {
							if (!getOne("select * from cart where u_no = ? and gNo = ?", user.get(0), game.get(0))
									.isEmpty()) {
								eMsg("장바구니에 있는 게임입니다.");
								return;
							}
							...
							int price = toInt(game.get(7)) <= uGd && toInt(game.get(6)) != 0 ? toInt(game.get(6))
									: toInt(game.get(5));
							...
							execute("insert into library values(0, ?, ?, ?, ?)", user.get(0), gNo, price,
									LocalDate.now());
			...
			var review = getRows("select * from review where u_no = ? and gNo = ?", user.get(0), gNo);
			var btn = btn("", a -> {
				...
				if (a.getActionCommand().equals("등록")) {
					...
					execute("update review set r_score=?, r_content=? where r_no = ?", score, txt.getText(), r_no);
				}
	...
	void setReview() {
		...
		for (var rs : getRows(
				"select u.u_no, u_img, u_name, r_score, r_content, r_no from review r, user u where r.u_no = u.u_no and gNo = ?",
				gNo)) {
