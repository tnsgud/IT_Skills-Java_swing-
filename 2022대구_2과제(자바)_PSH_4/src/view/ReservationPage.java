...
public class ReservationPage extends BasePage {
	...
	private void ui() {
		...
		for (var r : rs) {
			...
			lblArea[i] = event(lbl(r.get(1).toString(), 0, 13), e -> {
				var me = lblArea[i];
				for (var l : lblArea) {
					l.setBorder(null);
				}
			...
			while (!sdate.isAfter(edate)) {
				...
				if (sdate.isAfter(LocalDateTime.now())) {
					...
					var left = 100 - getRows("select * from reservation where r_time = ? and r_date = ? and m_no = ?",
							sdate.toLocalTime().format(DateTimeFormatter.ofPattern("HH:mm")), date, m.get(0)).stream()
							.flatMap(a -> Arrays.asList(a.get(6).toString().split("\\.")).stream()).distinct().count();
	...
	private void addTheater(String a_no) {
		...
		for (var r : getRows("select * from theater where a_no = ?", a_no)) {
			var lbl = event(lbl(r.get(1).toString(), 0, 13), e -> {
				if (e.getButton() == 1) {
