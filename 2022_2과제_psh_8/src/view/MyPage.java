...
public class MyPage extends BaseFrame {
	...
	public MyPage() {
		...
		n.add(btn("삭제", a -> {
			...
			iMsg("삭제가 완료되었습니다.");
			execute("delte from reservation where r_no=?", t.getValueAt(t.getSelectedRow(), 6));
		}), "East");
		nw.add(lbl("날짜 :", 2, 12));
		nw.add(com);
	...
	private void data() {
		...
		var rs = rs(
				"select r_date, r_time, c_name, t_name, r_people, format(r_people * c_price, '#,##0') from cafe c, reservation r, theme t where t.t_no=r.t_no and r.c_no = c.c_no and u_no=? "
