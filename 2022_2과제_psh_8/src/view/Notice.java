...
public class Notice extends BaseFrame {
	...
	public Notice() {
		...
		en.add(save = btn("수정", a -> {
			execute("update notice set n_content=?, n_title =? where n_no=?", area.getText(), title.getText(), t.getValueAt(t.getSelectedRow(), 0));
		}), "East");
		...
		t.addMouseListener(new MouseAdapter() {
			public void mousePressed(java.awt.event.MouseEvent e1) {
				...
				if (e.isVisible() && !isMe) {
					...
					execute("update notice set n_viewcount=n_viewcoutn+1 where n_no=?",
