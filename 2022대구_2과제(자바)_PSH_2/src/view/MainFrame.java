	...
	private void ui() {
		...
		for (int i = 0; i < cap.length; i++) {
			...
			ns.add(hyplbl(cap[i], 0, 15, e -> {
				...
				if (text.equals(cap[0])) {
					...
					if (BasePage.user == null) {
						if (loginCheck() == JOptionPane.YES_OPTION) {
							new LoginFrame().addWindowListener(new Before(MainFrame.this));
							return;
						} else {
							return;
						}
				...
				BasePage.cf.addWindowListener(new Before(MainFrame.this));
		...
		for (var r : rs) {
			...
			img.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					...
					new MovieInfoDialog(getRows(sql, r.get(0)).get(0));
				}
			});
