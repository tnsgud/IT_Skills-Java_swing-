...
public class AirlineTicket extends BasePage {
	...
	public void addRow() {
		...
		for (var sno : sNo) {
			var rs = getRows(
					"select s_no, a1_name, a2_name, time_format(s_time, '%H:%m'), round(st_distance_sphere(point(a1_longitude, a1_latitude), point(a2_longitude, a2_latitude)) * 0.001, -2), format(s_price, '#,##0'), 140-(select count(*) from reservation r, companion c where r.r_no = c.r_no and r.s_no = v1.s_no and r_date =?)  from v1 where v1.s_no = ?",
					date.toString(), sno).get(0);
			var sTime = LocalTime.parse(rs.get(3).toString());
			var distance = (int) Double.parseDouble(rs.get(4).toString());
			sTime = sTime.plusHours(distance / 600);
			sTime = sTime.plusMinutes(distance % 600 / 10);
			rs.set(4, sTime);
