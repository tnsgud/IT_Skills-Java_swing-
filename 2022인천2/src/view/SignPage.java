...
public class SignPage extends BasePage {
	...
	public SignPage() {
		...
		for (int i = 0; i < cap.length; i++) {
			...
			if (i == 6) {
				c.add(com = new JComboBox<String>(rs("select name from building where type = 2").stream()
						.flatMap(a -> a.stream()).toArray(String[]::new)));
			} else {
				c.add(txt[i] = new JTextField());
			}
		}
		c.add(hyplbl("이미 계정이 있으십니까?", 2, 0, 15, Color.orange, () -> mf.swapPage(new LoginPage())));
		c.add(btn("회원가입", a -> {
			for (var t : txt) {
		...
		txt[5].addMouseListener(new MouseAdapter() {
			public void mousePressed(java.awt.event.MouseEvent e) {
				new DatePicker(txt[5], LocalDate.now(), true).show(txt[5], 0, txt[5].getHeight());
			};
		});
