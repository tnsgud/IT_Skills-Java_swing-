...
public class LookUp extends BaseDialog implements Tool {
	...
	private void data() {
		m.setRowCount(0);
		var rs = DB.rs(
				"select s.no, concat(v1.l11name, ' ', v1.l21name), concat(v1.l12name, ' ', v1.l22name), time_format(addtime(s.date, s.elapsed_time), '%H:%i:%s'), s.date from v1, schedule s, reservation r where r.user_no = ? and s.no = v1.sno and s.no = r.schedule_no order by s.date asc",
				BaseFrame.no);
		try {
			while (rs.next()) {
