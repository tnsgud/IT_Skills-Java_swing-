...
public class Sign extends BaseDialog implements Tool {
	...
	private void ui() {
		...
		add(btn("회원가입", a -> {
			...
			if (!DB.getOne("select * from user where id=?", txt[1].getText()).isEmpty()) {
				eMsg("Id가 중복되었습니다.");
				return;
			}

			if (!DB.getOne("select * from user where email=?", txt[4].getText()).isEmpty()) {
