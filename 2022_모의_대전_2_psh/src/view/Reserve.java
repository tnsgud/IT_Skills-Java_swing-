...
public class Reserve extends BaseFrame implements Tool {
	String[] cap = "장소,출연,가격,날짜".split(",");
	DefaultTableModel m = model("p_no,날자,여유좌석".split(","));
	JTable t = table(m);
	...
	private void data() {
		var rs = DB.rs(
				"select p.p_no, date_format(p_date, '%m, %d. ') as date, t_seat from perform p. ticket t where p.p_no=t.p_no and p_name like ?",
				"%" + DB.getOne("select p_name from perform where p_no=?", pno) + "%");
		var p_no = 0;
		var date = "";
		var t_seat = 60;
		try {
	...
	private void ui() {
		...
		add(lbl(DB.getOne("select p_name from perform where p_no=?", pno), 2, 35), "North");
		add(c = new JPanel(new BorderLayout(5, 5)));
		add(sz(scr, 150, 150), "East");
		add(s, "South");

		c.add(img("공연사진/" + pno + ".jpg", 150, 150), "West");
		c.add(c_c);
		...
		s.add(sz(btn("예매하기", a -> {
			if(uno == 0 ) {
				int an = JOptionPane.showConfirmDialog(null, "회웜나 가능한 서비스입니다.\n로그인 하시겠습니까?", "로그인", JOptionPane.YES_NO_OPTION);
	...
	private void addlbl(JPanel p) {
		...
		var rs = DB.rs("select p_place, p_actor, format(p_price, '#,##0'), p_date from perform where p_no=?",
