...
public class Search extends BaseFrame implements Tool {
	String[] icap = "전체,뮤지컬,오페라,콘서트".split(","), name = ",M,O,C".split(",");
	West west;
	...
	private void event() {
		for (var i : item) {
			i.addActionListener(a -> {
				west.tit.setText("분류: " + a.getActionCommand());
	...
	private void ui() {
		...
		for (int i = 0; i < icap.length; i++) {
			menu.add(item[i] = new JMenuItem(icap[i]));
